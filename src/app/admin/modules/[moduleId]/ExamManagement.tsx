...
import { createExamAction, addExamQuestionAction, deleteExamQuestionAction } from '../detailActions'
      ...
      {exam ? (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            ...
            {/* Hapus Ujian bisa ditambahkan nanti jika diperlukan */}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '2rem' }}>
            ...
            {exam.exam_questions?.map((q: any, idx: number) => (
              <div key={q.id} style={{ padding: '0.8rem 1rem', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '0.9rem', fontWeight: 600 }}>{idx + 1}. {q.question}</p>
                  <p style={{ fontSize: '0.75rem', color: 'var(--color-success)', marginTop: '0.2rem' }}>Jawaban Benar: {q.correct_answer.toUpperCase()}</p>
                </div>
                <button 
                    onClick={() => { if (confirm('Hapus soal ini?')) startTransition(async () => { await deleteExamQuestionAction(q.id) }) }} 
          ...
          <div style={{ padding: '1.25rem', background: '#fffbeb', borderRadius: '12px', border: '1px solid #fde68a' }}>
            ...
            <form action={async (fd) => { startTransition(async () => { await addExamQuestionAction(fd) }) }}>
        ...
        <form action={async (fd) => { startTransition(async () => { await createExamAction(fd) }) }}>
